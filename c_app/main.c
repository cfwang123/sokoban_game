/* main.c — 推箱子 C 命令行版主程序 */
#define WIN32_LEAN_AND_MEAN
#include <windows.h>   /* DWORD, GetTickCount, VK_* */
...
static void render_front(void) {
    /* ---- 更新窗口标题（纯 ASCII，避免 CJK 在标题栏渲染不一致） ---- */
    wchar_t title[128];
    const char *lvlName = "";
...
int main(void) {
    ...
    if (levels_load("levels.json", &g_levels) != 0) {
        fprintf(stderr, "无法加载 levels.json，请确保该文件与 sokoban.exe 同目录。\n");
        return 1;
    }
    ...
    for (;;) {
        ...
        if (type == EV_KEY) {
            ...
            g_resizePending = 1;
            g_resizeTick = GetTickCount();
        }

        /* 处理延迟的 resize */
        if (g_resizePending && (int)(GetTickCount() - g_resizeTick) >= 500) {
            g_resizePending = 0;
            con_clear();
            invalidate_back();
            render_front();

...
A classic Sokoban puzzle with **135** implementations: C consoles, 2D/3D web, desktop GUIs, mobile, embedded, handheld homebrew, editor/browser extensions, plus teaching ports in dozens of languages.

Each teaching folder has bilingual docs: `README.md` (English) and `README.ZH.md` (Chinese).

## Features

- Classic Sokoban gameplay with many levels (`levels.json`)
- **C console** — `c_app/` (Windows) and `sokoban_linux/` (ncursesw)
- **Multi-language CLIs** — Python / PHP / Lua / Node.js / Ruby / Java / C# / Kotlin / Perl / R / Haskell / Rust / Go / Zig / C++ / PowerShell / Bash / CMD / …
...
- **Esoteric languages** — Brainfuck / Befunge / JSFuck / Unreadable / Parenthesis Hell (playable); others: [docs/UNSUPPORTED_LANGS.md](docs/UNSUPPORTED_LANGS.md))
- **Desktop demos** — Pygame / Qt / Electron / SDL2 / Godot / raylib / Win32 / MFC / Tkinter / PyQt / WinForms / WPF / Avalonia / C# TUI / MAUI / WinUI3 / Blazor
- **2D / 3D web** — `html_app/` / `html_3dapp/` (3D uses three.js) · React / Vue / Angular teaching · Cocos2d-style
...
├── cppapp1/             # C++ CLI (C++17)
├── cpp26app1/           # C++03–C++26 syntax museum (-std=c++26)
├── cpphardapp1/         # intentionally unreadable C++ (anti-example)
...
├── unreadableapp1/      # Unreadable (pure quote game + interpreter)
├── parenthesishellapp1/ # Parenthesis Hell (interpreter + PH-value state)
...
├── docs/                # Languages not implemented (policy + reasons)
...
Open `html_app/index.html` in any modern browser. No server required.
...
See **[TODO.md](TODO.md)**. Each teaching folder has bilingual docs: `README.md` (English) + `README.ZH.md` (Chinese). Includes Safari, Emacs, JetBrains plugins, browser extensions, consoles, handhelds, etc.

### C Console Version (Windows)
...
Requires a C compiler (e.g., GCC / MinGW on Windows). Copies `../levels.json` next to the exe.

### C Console Version (Linux)
...
Needs `gcc` and `ncursesw` (`libncursesw5-dev` / `ncurses-devel`).
...
cd unreadableapp1        && python -X utf8 main.py   # pure Unreadable + interpreter
cd parenthesishellapp1   && python -X utf8 main.py   # PH interpreter + playable host
# not implementable: docs/UNSUPPORTED_LANGS.md
# … every teaching folder has README.md + README.ZH.md
```
...
| [flutterapp1](flutterapp1/README.md) | Flutter teaching sources; `flutter run` if SDK installed |
| [unityapp1](unityapp1/README.md) | Unity C# teaching scripts; open in Unity Editor |
